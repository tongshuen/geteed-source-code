...
class TextEditor:
       ...
        
        ...
        if os.path.exists(filename):
            try:
                ...
                self.show_message(_(f"Error loading file: {str(e)}"), 3)
    ...
    def load_extensions(self) -> Dict[str, Callable]:
        ...
        if os.path.exists(ext_path):
            try:
                ...
                self.show_message(_(f"Error loading extensions: {str(e)}"), 3)
        return extensions
    
   ...
    
    def save_file(self) -> bool:
        if self.read_only:
            self.show_message(_("Read-only mode - cannot save"), 2)
            return False
        
        try:
            with open(self.filename, 'w', encoding=self.encoding) as f:
                f.write('\n'.join(self.lines))
            self.modified = False
            self.show_message(_(f"Saved {self.filename}"), 2)
            return True
        except Exception as e:
            self.show_message(_(f"Error saving file: {str(e)}"), 3)
            return False
    
       ...
        
        if self.current_language == 'python':
            try:
                ...
                self.modified = True
                self.show_message(_("Auto-formatted with autopep8"), 2)
            except ImportError:
                self.show_message(_("autopep8 not installed - cannot auto-format"), 2)
        else:
            self.show_message(_(f"Auto-format not supported for {self.current_language}"), 2)
    
    def new_file(self):
        if self.modified:
            self.show_message(_("Unsaved changes! Press Ctrl+S to save or Ctrl+N again to discard"), 2)
       ...
        
        ...
        self.read_only = False
        self.current_language = self.detect_language()
        self.show_message(_("New file created"), 2)
    
    def open_file(self, stdscr):
        if self.modified:
            self.show_message(_("Unsaved changes! Press Ctrl+S to save or Ctrl+O again to discard"), 2)
            key = stdscr.getch()
            if key == 19:  # Ctrl+S
                if not self.save_file():
                    return
        
        self.show_message(_("Enter filename to open: "), 0)
        stdscr.refresh()
        
        curses.echo()
        ...
        if filename:
            try:
                with open(filename, 'r', encoding=self.encoding) as f:
                    ...
                    self.read_only = False
                    self.current_language = self.detect_language()
                    self.show_message(_(f"Opened {filename}"), 2)
            except Exception as e:
                self.show_message(_(f"Error opening file: {str(e)}"), 3)
    
    def handle_input(self, stdscr):
       ...
        
        ...
        if key == curses.KEY_UP:
            ...
            if self.modified:
                self.show_message(_("Unsaved changes! Press Ctrl+S to save or Ctrl+Q again to quit"), 2)
                key = stdscr.getch()
                if key == 19:  # Ctrl+S
                    if self.save_file():
                        return False
            ...
            self.clipboard = [self.lines[self.cursor_y]]
            self.show_message(_("Line copied to clipboard"), 1)
            ...
            self.show_message(_(f"Line wrap {'enabled' if self.line_wrap else 'disabled'}"), 2)
        elif key == 26:  # Ctrl+Z - Undo
            self.show_message(_("Undo - Not implemented"), 2)
        elif key == 21:  # Ctrl+U - Redo
            self.show_message(_("Redo - Not implemented"), 2)
            ...
            if self.macro_recording:
                self.macro_recording = False
                self.show_message(_("Macro recording cancelled"), 2)
   ...
    
    def toggle_bookmark(self):
        if self.cursor_y in self.bookmarks:
            ...
            self.show_message(_(f"Bookmark removed at line {self.cursor_y + 1}"), 2)
        else:
            self.bookmarks[self.cursor_y] = (self.cursor_x, self.cursor_y)
            self.show_message(_(f"Bookmark added at line {self.cursor_y + 1}"), 2)
    
    def goto_bookmark(self, stdscr):
        if not self.bookmarks:
            self.show_message(_("No bookmarks set"), 2)
            return
        
   ...
    
    def toggle_macro_recording(self):
        ...
        if self.macro_recording:
            self.macro_commands = []
            self.show_message(_("Macro recording started"), 2)
        else:
            self.show_message(_(f"Macro recorded ({len(self.macro_commands)} commands)"), 2)
    
    def run_macro(self):
        if not self.macro_commands:
            self.show_message(_("No macro recorded"), 2)
            return
        
        for cmd in self.macro_commands:
            self.handle_input(curses.initscr())  # 简化处理
        
        self.show_message(_(f"Macro executed ({len(self.macro_commands)} commands)"), 2)
    
    def show_find_dialog(self, stdscr):
        self.show_message(_("Find: ") + self.search_term)
        stdscr.refresh()
        
        curses.echo()
       ...
        
        if term:
            ...
            if pos is not None:
                ...
                self.show_message(_(f"Found: {self.search_term}"), 2)
            else:
                self.show_message(_(f"Not found: {self.search_term}"), 2)
    
    def show_replace_dialog(self, stdscr):
        self.show_message(_(f"Replace '{self.search_term}' with: {self.replace_term}"))
        stdscr.refresh()
        
        curses.echo()
       ...
        
        if replace is not None:
            ...
            if self.replace_next(self.search_term, self.replace_term):
                self.show_message(_(f"Replaced: {self.search_term} -> {self.replace_term}"), 2)
            else:
                self.show_message(_(f"Not found: {self.search_term}"), 2)
    
    def show_goto_dialog(self, stdscr):
        self.show_message(_("Go to line: "))
        stdscr.refresh()
        
        curses.echo()
   ...
    
    def show_help(self, stdscr):
        help_text = [
            _("GETEED (General Text Editor) Help"),
            "--------------------------------",
            _("Navigation:"),
            _("  Arrow Keys    - Move cursor"),
            _("  Home/End      - Move to start/end of line"),
            _("  Ctrl+A/Ctrl+E - Move to start/end of line"),
            _("  Page Up/Dn    - Move up/down one screen"),
            _("  Ctrl+L        - Go to line number"),
            "",
            _("Editing:"),
            _("  Enter         - Insert new line"),
            _("  Backspace     - Delete previous character"),
            _("  Delete        - Delete next character"),
            _("  Tab           - Insert tab/spaces"),
            _("  Ctrl+D        - Duplicate line"),
            _("  Ctrl+]/Ctrl+/ - Indent right/left"),
            _("  Ctrl+^        - Auto format"),
            "",
            _("Clipboard:"),
            _("  Ctrl+X        - Cut line"),
            _("  Ctrl+C        - Copy line"),
            _("  Ctrl+V        - Paste"),
            _("  Ctrl+K        - Cut to end of line"),
            _("  Ctrl+Y        - Paste from clipboard"),
            "",
            _("Search & Replace:"),
            _("  Ctrl+F        - Find"),
            _("  Ctrl+R        - Replace"),
            "",
            _("Bookmarks:"),
            _("  Ctrl+B        - Toggle bookmark"),
            _("  Ctrl+G        - Go to bookmark"),
            "",
            _("Macros:"),
            _("  Ctrl+M        - Toggle macro recording"),
            _("  Ctrl+\\        - Run macro"),
            _("  ESC           - Cancel recording"),
            "",
            _("File Operations:"),
            _("  Ctrl+S        - Save file"),
            _("  Ctrl+Q        - Quit"),
            ...
            "",
            _("View Options:"),
            _("  Ctrl+T        - Toggle line numbers"),
            _("  Ctrl+W        - Toggle line wrap"),
            "",
   ...
    
    def render(self, stdscr):
       ...
        
        ...
        status += _("Modified") if self.modified else _("Saved")
        status += " - " + (_("READ ONLY") if self.read_only else _("READ/WRITE"))
...
def main(stdscr):
   ...
    
    # 解析命令行参数
    if len(sys.argv) < 2:
        stdscr.addstr(0, 0, _(f"Usage: {sys.argv[0]} <filename> [--nl] [--ro] [--light] [--dark]"))
        stdscr.refresh()
        stdscr.getch()
        return
    
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(_(f"Usage: {sys.argv[0]} <filename> [--nl] [--ro] [--light] [--dark]"))
        print(_("Options:"))
        print(_("  --nl      Disable line numbers"))
        print(_("  --ro      Read-only mode"))
        print(_("  --light   Light color theme"))
        print(_("  --dark    Dark color theme (default)"))
        sys.exit(1)
    
    try:
        curses.wrapper(main)
